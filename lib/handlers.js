...
const storeComment = (body) => {
  ...
  writeFileSync(STORAGE_FILE, JSON.stringify(comments));
};
...
const formateSingleComment = function (commentInfo) {
  const {usrName, comment, date} = commentInfo;
  const cmt = new Comment(usrName, comment, date);
  return cmt.toHTMLString();
};
